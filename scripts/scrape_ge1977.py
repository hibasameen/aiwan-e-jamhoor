#!/usr/bin/env python3
"""
Scrape the 1977 (non-party) National Assembly results from ElectionPakistani.
Same shape and logic as scrape_ge1988.py; 200 seats (NA-1..200).

    python3 scripts/scrape_ge1977.py            # all 207 seats
    python3 scripts/scrape_ge1977.py 1 20       # a range

Writes data/results_1977/na_1977_candidates.csv and na_1977_constituency.csv.
1977 was a PPP swept most seats; the PNA opposition alleged rigging (Zia coup followed in July 1977).
...
BASE = 'https://www.electionpakistani.com/ge1977/NA-{}.htm'
OUT = 'data/results_1977'
...
def main(lo=1, hi=200):
    ...
    with open(f'{OUT}/na_1977_candidates.csv', 'w', newline='', encoding='utf-8') as f:
        wr = csv.DictWriter(f, fieldnames=['na', 'candidate_name', 'party', 'votes', 'rank'])
        wr.writeheader(); wr.writerows(cand_rows)
    with open(f'{OUT}/na_1977_constituency.csv', 'w', newline='', encoding='utf-8') as f:
...
if __name__ == '__main__':
    a = [int(x) for x in sys.argv[1:3]] if len(sys.argv) > 2 else [1, 200]
    main(*a)
