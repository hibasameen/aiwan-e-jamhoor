#!/usr/bin/env python3
"""
Scrape the 1990 National Assembly results from ElectionPakistani, in the same
shape as data/results_2024/.

    python3 scripts/scrape_ge1990.py            # all 207 seats
    python3 scripts/scrape_ge1990.py 1 20       # a range, for a trial run

Writes data/results_1990/na_1990_candidates.csv and na_1990_constituency.csv.
Pages carry candidate names, parties and votes but no registered electorate or
turnout, so those columns are left blank — the same situation as 2024.

Be polite: the default delay is 1s per page, so a full run takes ~4 minutes.
"""
import sys, os, re, time, csv, io
import urllib.request

BASE = 'https://www.electionpakistani.com/ge1990/NA-{}.htm'
OUT = 'data/results_1990'
...
def main(lo=1, hi=207):
    ...
    with open(f'{OUT}/na_1990_candidates.csv', 'w', newline='', encoding='utf-8') as f:
        wr = csv.DictWriter(f, fieldnames=['na', 'candidate_name', 'party', 'votes', 'rank'])
        wr.writeheader(); wr.writerows(cand_rows)
    with open(f'{OUT}/na_1990_constituency.csv', 'w', newline='', encoding='utf-8') as f:
