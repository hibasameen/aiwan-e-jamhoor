...
BASE = {
    ...
    "about.html": ("About Aiwan-e-Jamhoor — Pakistan Election Data", "About Aiwan-e-Jamhoor, Hiba Sameen's independent archive of Pakistan's National Assembly elections, part of Adaad."),
...
def metadata(title, desc, path, extra=None):
    url = ORIGIN + path
    graph = [{"@type": "WebPage", "@id": url + "#page", "url": url, "name": title, "description": desc, "inLanguage": "en", "isPartOf": {"@id": ORIGIN + "/#website"}}]
    if path == "/":
        graph.append({"@type": "WebSite", "@id": ORIGIN + "/#website", "url": url, "name": "Aiwan-e-Jamhoor", "alternateName": ["Aiwan e Jamhoor", "ایوانِ جمہور"], "description": desc, "creator": {"@type": "Person", "name": "Hiba Sameen", "url": "https://adaad.org/about/"}, "publisher": {"@type": "Organization", "name": "Adaad", "url": "https://adaad.org/"}})
    if extra:
...
def patch_metadata(file, title, desc, path):
    ...
    text = text.replace("</head>", metadata(title, desc, path) + "\n</head>")
    ...
    if 'href="/elections/"' not in text:
        text = text.replace('<nav class="main">', '<nav class="main"><a href="/elections/">Results archive</a>', 1)
...
<meta property="og:type" content="website"><meta property="og:site_name" content="Aiwan-e-Jamhoor"><meta name="twitter:card" content="summary">
<link rel="stylesheet" href="/fonts/fonts.css"><link rel="stylesheet" href="/assets/research.css">
</head><body><a class="skip" href="#content">Skip to results</a><header><a class="brand" href="/">Aiwan-e-Jamhoor <span lang="ur" dir="rtl">ایوانِ جمہور</span></a><nav aria-label="Main"><a href="/elections/">Results archive</a><a href="/map.html">Election map</a><a href="/candidates.html">Candidates</a><a href="/method.html">Sources &amp; method</a></nav></header>
<main id="content">{body}</main><footer>By <a href="https://adaad.org/about/">Hiba Sameen</a> · <a href="https://adaad.org/">Adaad: Pakistan data journalism</a> · <a href="https://darbar.adaad.org/">Data Darbar</a><p>Derived data: <a href="{LICENSE}">CC BY 4.0</a>, subject to <a href="{LICENSE_NOTES}">upstream terms, including GPL for Cookman-derived results</a>. Credit the original sources and retain the result-vintage notes.</p></footer><script src="/assets/analytics.js" defer></script></body></html>''')


def dataset(name, desc, path, download, year):
    return {"@type": "Dataset", "name": name, "description": desc + " " + VINTAGE, "url": ORIGIN + path, "creator": {"@type": "Person", "name": "Hiba Sameen", "url": "https://adaad.org/about/"}, "license": LICENSE_NOTES if year in COOKMAN_YEARS else LICENSE, "isAccessibleForFree": True, "spatialCoverage": "Pakistan", "temporalCoverage": year, "isBasedOn": ORIGIN + "/method.html", "distribution": {"@type": "DataDownload", "encodingFormat": "text/csv", "contentUrl": ORIGIN + download}}
